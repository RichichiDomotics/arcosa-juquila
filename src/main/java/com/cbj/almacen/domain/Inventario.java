...
@org.hibernate.annotations.NamedQueries({
        @org.hibernate.annotations.NamedQuery(name="Inventario.getByClienteConsecutivo",
                query="SELECT c FROM Inventario c where c.idCliente = :idCliente and c.consecutivo = :consecutivo order by c.renglon"),
        ...
        @org.hibernate.annotations.NamedQuery(name = "Inventario.getSaldoXTunel",
                query = "SELECT c.camara, sum(c.cantidadInventario),sum(c.cantidadInventario*c.pesou), sum((c.cantidadInventario*c.pesou)/1000) from Inventario c WHERE c.cantidadInventario >0 AND c.camara IN ('12','T1','T2','T3') GROUP BY c.camara"),
        ...
        query = "SELECT i.camara,c.ID_CLIENTE, c.NOM_CLIENTE,i.consecutivo,i.renglon,i.fechaCaptura, i.cantidadInventario,i.pesou,i.cantidadInventario*i.pesou,i.descripcion,i.embalaje,i.marca from Inventario i, Clientes c WHERE i.cantidadInventario >0 and i.idCliente = c.ID_CLIENTE"),
        @org.hibernate.annotations.NamedQuery(name = "Inventario.getCitroFrut",
        query = "SELECT i FROM Inventario i WHERE idCliente in (186,326,2361) AND consecutivo in (:consecutivo)"),

        @org.hibernate.annotations.NamedQuery(name = "Inventario.getSaldosTotales",
                query = "SELECT SUM(c.cantidadInventario * c.pesou)AS saldo, sum (c.cantidadInventario) as piezas  FROM Inventario c"),

        @org.hibernate.annotations.NamedQuery(name = "Inventario.getPosicionesProbocaV",
                query = "select SUM(c.cantidadInventario)/60 as posisiciones from Inventario c where c.cantidadInventario>0 and c.idCliente=333 and SUBSTR(c.marca,-1)='V' and c.descripcion not like '%ARRACHERA%'"),

        @org.hibernate.annotations.NamedQuery(name = "Inventario.getPosicionesProbocaI",
                query = "select SUM(c.cantidadInventario)/25 as posisiciones from Inventario c where c.cantidadInventario>0 and c.idCliente=333 and SUBSTR(c.marca,-1)='I'"),

        @org.hibernate.annotations.NamedQuery(name = "Inventario.getPosicionesProbocaC2",
                query = "select SUM(c.cantidadInventario)/25 as posisiciones from Inventario c where c.cantidadInventario>0 and c.idCliente=333 and c.marca in ('2145C','2146C','2147C')"),

        @org.hibernate.annotations.NamedQuery(name = "Inventario.getPosicionesProbocaArrachera",
                query = "select SUM(c.cantidadInventario)/60 as posisiciones from Inventario c where c.cantidadInventario>0 and c.idCliente=333 and c.descripcion like '%ARRACHERA%'")
})
