...
@Controller
@Scope("prototype")
@RequestMapping("/exportSalPorCamara")
public class ExportSalPorCamaraController {
    ...
    public void export(HttpServletRequest request, HttpServletResponse response,
                       ...
                       @RequestParam(value = "idCamara") String idCamara
        ...
        try {
            JasperReportBuilder jrb = createJasperReport(fechaInicio,fechaFin,idCamara);
    ...
    private JasperReportBuilder createJasperReport(String fechaInicio,String fechaFin, String idCamara) {
        ...
        try {
            ...
            TextColumnBuilder<Double> quantityPza = col.column("PZAS", "cantidadSalida", type.doubleType()).setWidth(55).setPattern("###,###,###");
            TextColumnBuilder<Double> quantityKgs = col.column("KILOS", "pesou", type.doubleType()).setWidth(55).setPattern("###,###,###.##");
            ...
            return report()//create new report design
                    ...
                    .columns(//add columns
                            ...
                            col.column("ID", "idCliente", type.stringType()).setWidth(55),
                            col.column("NOMBRE CLIENTE", "descripcion", type.stringType()).setWidth(255),
                            col.column("RD", "consecutivo", type.integerType()).setWidth(55).setPattern("##########"),
                    ...
                    .title(cmp.horizontalList()
                            .add(cmp.image(url).setFixedDimension(80, 80),
                                    cmp.verticalList()
                                            .add(
                                                    cmp.text("GRUPO ARCOSA PLANTA JUQUILA").setStyle(titleStyle).setWidth(310).setHorizontalAlignment(HorizontalAlignment.LEFT),
                    ...
                    .setDataSource(createDataSource(fechaInicio, fechaFin, idCamara))//set datasource
    ...
    private JRDataSource createDataSource(String fechaInicio,String fechaFin,String idCamara) {
        ...
        if(idCamara.trim().equalsIgnoreCase("")) {
            oList = this.salidasDetalleManager.getReporteByCamaraVacia(dateInicio, dateFin);
        }else{
            oList = this.salidasDetalleManager.getReporteByCamara(dateInicio, dateFin, idCamara);
        }
        ...
        for (Object[] o : res) {
            final SalidasDetalle salidasDetalle = new SalidasDetalle();
                salidasDetalle.setIdCliente((String) o[0]);
                salidasDetalle.setDescripcion((String) o[1]);
                salidasDetalle.setConsecutivo((Integer) o[2]);
                salidasDetalle.setCantidadSalida((Double) o[3]);
                salidasDetalle.setPesou((Double) o[4]);
                salidasDetalle.setFolioSalida((Integer) o[5]);
