...
public class EntradaRdDaoImpl implements DetalleRdDao {
    ...
    @Transactional(readOnly = false)
    public boolean insertListaEntradas(List<DetallesRd> detallesRds){
        boolean respuesta = false;
        try{
            EntityManagerFactory emf = Persistence.createEntityManagerFactory("springappPU");
            em = emf.createEntityManager();
            em.getTransaction().begin();
            for(Iterator<DetallesRd> iterator = detallesRds.iterator(); iterator.hasNext();){
                DetallesRd entrada = iterator.next();
                em.merge(entrada);
                em.flush();
                em.clear();
            }
            em.getTransaction().commit();
            //em.merge(detallesRds);
            respuesta=true;
        }catch (Exception e){
    ...
    public List<Object> getRDAlmacenajesNuevos(String idcliente, Date fechaini, Date fechafin){
        try{
            ...
            Query query = em.createNativeQuery("SELECT d.idCliente as cliente, d.consecutivo as rd , date_format(d.fechaCaptura, '%Y-%m-%d') as iniper," +
                    "CASE WHEN c.periodo=30 THEN date_format(DATE_SUB(DATE_ADD(d.fechaCaptura,INTERVAL 1 MONTH),INTERVAL 1 DAY),'%Y-%m-%d')  " +
                    "WHEN DATE_FORMAT(LAST_DAY(curdate()),'%d') = '31' and c.periodo = 15 and DATE_FORMAT(d.fechaCaptura,'%d') >16 THEN date_format(DATE_ADD(d.fechaCaptura,INTERVAL c.periodo DAY),'%Y-%m-%d') " +
                    "WHEN DATE_FORMAT(LAST_DAY(curdate()),'%d') = '31' and c.periodo = 20 and DATE_FORMAT(d.fechaCaptura,'%d') >11 THEN date_format(DATE_ADD(d.fechaCaptura,INTERVAL c.periodo DAY),'%Y-%m-%d')" +
                    "ELSE date_format(DATE_ADD(d.fechaCaptura, INTERVAL (c.periodo-1) DAY), '%Y-%m-%d') END as finper, d.camara as camara, (select ROUND(SUM(cantidad*pesou),2) from detalles_rd where claveProducto=c.claveProducto AND consecutivo = d.consecutivo)as kilos" +
    ...
    public List<Object> getRDTunelesNuevos(String idcliente, Date fechaini, Date fechafin){
        try{
            ...
            Query query = em.createNativeQuery("SELECT d.idCliente as cliente, d.consecutivo as rd , date_format(d.fechaCaptura, '%Y-%m-%d') as iniper," +
                    ...
                    "and d.idCliente in ( "+idcliente+" ) and d.idCliente not in (341,428,536,109,116,194,196,394,187,314,258,387,396) GROUP BY d.consecutivo");
            //query.setParameter("idcliente", idcliente);
            query.setParameter("fechaini", fechaini ,TemporalType.DATE);
            query.setParameter("fechafin", fechafin, TemporalType.DATE);
            return query.getResultList();
    ...
    public List<Object> getRDMEYS(String idcliente, Date fechaini, Date fechafin){
        try{
            ...
            //String query="SELECT d.idCliente, d.consecutivo, date_format(d.fechaCaptura, '%Y-%m-%d'),date_format(d.fechaCaptura, '%Y-%m-%d')  +  INTERVAL (c.periodo-1) DAY, d.camara, round(sum(d.cantidad * d.pesou),2),'ALM',c.cuotaAlmacenaje, round(sum(d.cantidad * d.pesou),2)*c.cuotaAlmacenaje,round(sum(d.cantidad * d.pesou),2)*c.cuotaAlmacenaje, '',curdate() FROM DetallesRd d, Convenios c WHERE c.claveProducto=d.claveProducto and date(d.fechaCaptura) >= :fechaini and date(d.fechaCaptura) <= :fechafin and d.idCliente= :idcliente GROUP BY d.consecutivo";
            Query query = em.createNativeQuery("SELECT d.idCliente as cliente, d.consecutivo as rd , date_format(d.fechaCaptura, '%Y-%m-%d') as iniper," +
                    "date_format(d.fechaCaptura, '%Y-%m-%d')" +
                    " as finper, d.camara as camara, (select ROUND(SUM(cantidad*pesou),2) from detalles_rd where claveProducto=c.claveProducto AND consecutivo = d.consecutivo)as kilos" +
                    ...
                    "and d.idCliente in ( "+idcliente+" ) and d.idCliente not in (109,116,194,196,394,396)  GROUP BY d.consecutivo");
            //query.setParameter("idcliente", idcliente);
            query.setParameter("fechaini", fechaini ,TemporalType.DATE);
            query.setParameter("fechafin", fechafin, TemporalType.DATE);
            return query.getResultList();
    ...
    public List<Object[]> getRendimientos(Date fechaini, Date fechafin) {
        try {
            Query e = this.em.createNativeQuery("SELECT         d.consecutivo,         d.camara,         DATE(d.fechaCaptura),         ROUND(SUM(d.cantidad * d.pesou), 2) AS kilos,         SUM(d.cantidad) AS piezas,         c.ID_CLIENTE,         c.NOM_CLIENTE,         d.posiciones,         e.cuotaAlmacenaje,         d.estibas,         CASE WHEN d.idCliente IN (         186,326,3261,101,1001,1003,340,492,242,529,535,557,56,563,575 ) THEN         ROUND(                 1.836 * (d.posiciones / d.estibas),                 2         ) ELSE         ROUND(                 1.53 * (d.posiciones / d.estibas),                 2         ) END AS superficie, ROUND(d.posiciones / d.estibas, 2) AS base, d.altura, d.almaen, CASE WHEN e.cuotaAlmacenaje < 1 THEN         'KILOGRAMO' ELSE         'TARIMA' END AS forma_cobro, e.periodo, CASE WHEN e.cuotaAlmacenaje < 1 THEN         ROUND(                 ROUND(                         (                                 ROUND(SUM(cantidad * pesou), 2)                         ),                         2                 ) * e.cuotaAlmacenaje,                 2         ) ELSE         ROUND(                 d.posiciones * e.cuotaAlmacenaje,                 2         ) END AS produccion_pesos, CASE WHEN e.cuotaAlmacenaje < 1 THEN         CASE                                    WHEN d.idCliente IN (                                                   186,326,3261,101,1001,1003,340,492,242,529,535,557,56,563,575                                       ) THEN                                                  ROUND(                                                                             (                         ROUND(                                 ROUND(SUM(cantidad * pesou)) * e.cuotaAlmacenaje,                                 2                                                                                                        )                                                                             ) / (                         1.836 * (                                 ROUND(d.posiciones / d.estibas, 2)                                                                                                                     )                                                                                             ),                 2                                                      )                                              ELSE                                                      ROUND(                                                                                             ROUND(                                                                                                                             ROUND(SUM(cantidad * pesou)) * e.cuotaAlmacenaje,                                                                                               2                                                                                            ) / (                         1.53 * (                                 ROUND(d.posiciones / d.estibas, 2)                                                                                                               )                                                                                             ),                                                                            2                                                            )                              END ELSE                            CASE                                      WHEN d.idCliente IN (                                                   186,326,3261,101,1001,1003,340,492,242,529,535,557,56,563,575                                       ) THEN                                                                 ROUND(                                                                                                             (                                 ROUND(                                        d.posiciones * e.cuotaAlmacenaje,                                        2                                 )                                                                                                  ) / (                                 1.836 * (                                        ROUND(d.posiciones / d.estibas, 2)                                 )                         ),                         2                                                                             )                                              ELSE                 ROUND(                                                                 ROUND(                         ROUND(d.posiciones * e.cuotaAlmacenaje,                         2                                                                           ) / (                         1.53 * (                                 ROUND(d.posiciones / d.estibas, 2)                                                                                                               )                                                                                             ),                                                                            2                                                            ),2                          )                              END END AS rendimiento_docto FROM         detalles_rd d,         clientes c,         convenios e WHERE d.idCliente = c.ID_CLIENTE AND d.claveProducto = e.claveProducto AND DATE(fechaCaptura) >= :fechaini AND DATE(fechaCaptura) <= :fechafin AND d.camara not in ('T1','T2','T3') GROUP BY         d.consecutivo ORDER BY         d.consecutivo;\n");
