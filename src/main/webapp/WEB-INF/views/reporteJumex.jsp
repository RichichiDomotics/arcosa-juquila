...
<table class="component Exportar_a_Excel">
    ...
    <tr bgcolor="438eba" style="color: #FFFFFF;">
        ...
        <th>Subtotal RD(Bruto)</th>
        <th>Total RD(Bruto)</th>
        <th>Subtotal RD(Neto)</th>
        <th>Total RD(Neto)</th>
    </tr>
    </thead>
    ...
    <c:set var="subtotalRDNeto" value="${0}"/>
    <c:set var="totalRenglonNeto" value="${0}"/>
    ...
    <c:forEach items="${reporte}" var="det">
        <tr>
            ...
            <td>${det.pesou}</td>
            <td>${det.pesoBruto}</td>
            <td><fmt:formatNumber  type="number" minFractionDigits="2" value="${det.pesou*det.cantidadInventario}"/></td>
            <td>${det.cantidadInventario}</td>
            <c:forEach items="${detalles}" var="c">
                <c:if test="${c.consecutivo == det.consecutivo && c.renglon == det.renglon}">
                    <td>${c.temperaturaAnden}</td>
                    <td>${c.almaen}</td>
                </c:if>
            </c:forEach>
            ...
            <c:set var="totalRenglonNeto" value="${(det.cantidadInventario*det.pesoBruto)}"/>
            <c:set var="subtotalRDNeto" value="${(subtotalRDNeto) + (totalRenglonNeto)}"/>
            ...
            <td><fmt:formatNumber  type="number" minFractionDigits="2" value="${totalRenglonNeto}"/></td>
            <td>
                <c:if test="${det.consecutivo != reporte[contador].consecutivo}">
                    <fmt:formatNumber type="number" minFractionDigits="2" value="${subtotalRDNeto}"/>
                    <c:set var="subtotalRDNeto" value="${0}"/>
                </c:if>
            </td>
...
<div id="totalInventario">Total Kilos (Brutos):<fmt:formatNumber type="number" maxFractionDigits="2"  value="${total}"/></div>
<div id="totalInventario">Total Kilos (Netos):<fmt:formatNumber type="number" maxFractionDigits="2"  value="${total}"/></div>
